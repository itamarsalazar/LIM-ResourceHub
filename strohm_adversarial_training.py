...
def adversarial_train(model, epsilon, prev_hist, train_loader, val_loader, init_epoch, last_epoch,
                      opt, clr,
    ...
    for epoch in range(init_epoch, last_epoch):
        ...
        with open(os.path.join(model_dir, 'epoch.txt'), 'w') as f:
            ...
            # Print some messages
            print(
                f"\nEnd of Epoch: {epoch} | Train Loss: {train_loss:.6f} "
...
if __name__ == '__main__':
    ...
    save_model_dir = os.path.join(this_dir, "models", f'strohm_adv_eps{str(eps)}')
    ...
    clr = CyclicLR(opt, base_lr=lr_base, max_lr=lr_max, step_size_up=2 * len(train_loader), mode='triangular2',
